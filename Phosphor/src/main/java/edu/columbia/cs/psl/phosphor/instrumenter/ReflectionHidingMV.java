...
public class ReflectionHidingMV extends MethodVisitor implements Opcodes {
    ...
    private boolean isUnsafeFieldGetter(int opcode, String owner, String name, Type[] args, String nameWithoutSuffix) {
        if(opcode != INVOKEVIRTUAL || !Instrumenter.isUnsafeClass(owner) || !name.endsWith(TaintUtils.METHOD_SUFFIX)) {
            return false;
        } else {
    ...
    private boolean isUnsafeCAS(String owner, String name, String nameWithoutSuffix) {
        if(!Instrumenter.isUnsafeClass(owner) || !name.endsWith(TaintUtils.METHOD_SUFFIX)) {
            return false;
        } else {
            if(Configuration.IS_JAVA_8){
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean isInterface) {
        ...
        if((disable || className.equals("java/io/ObjectOutputStream") || className.equals("java/io/ObjectInputStream")) && owner.equals("java/lang/Class") && !owner.equals(className) && name.startsWith("isInstance$$PHOSPHORTAGGED")) {
            ...
            if (this.methodName.startsWith("setObjFieldValues")
                    && Instrumenter.isUnsafeClass(owner)
                    && ((name.startsWith("putObject") || name.startsWith("compareAndSwapObject")
                            || name.startsWith("putReference") || name.startsWith("compareAndSetReference")))
            ) {
            ...
            } else if(this.methodName.startsWith("getObjFieldValues") && Instrumenter.isUnsafeClass(owner) && (name.startsWith("getObject") || name.startsWith("getReference"))) {
                desc = "(L" + owner + ";" + desc.substring(1);
                super.visitMethodInsn(Opcodes.INVOKESTATIC, getRuntimeUnsafePropogatorClassName(), name, desc, false);
            } else if((this.methodName.startsWith("getPrimFieldValues") || this.methodName.startsWith("setPrimFieldValues")) && Instrumenter.isUnsafeClass(owner) && (name.startsWith("put") || name.startsWith("get"))) {
                name = name + "$$NOUNBOX";
            ...
            switch(owner) {
                case "java/lang/invoke/MethodType":
                    if(name.startsWith("lastParameterType")){
                        desc = "(L" + owner + ";" + desc.substring(1);
                ...
                case "java/lang/reflect/Method":
                    if(name.startsWith("invoke")) {
                        maskMethodInvoke();
                    } else if(name.startsWith("get") && !className.equals(owner) && !className.startsWith("sun/reflect") && !className.startsWith("jdk/internal/reflect") && !className.startsWith("java/lang/Class")) {
                ...
                case "java/lang/reflect/Constructor":
                    ...
                    } else if(name.startsWith("get") && !className.equals(owner) && !className.startsWith("sun/reflect") && !className.startsWith("jdk/internal/reflect") && !className.equals("java/lang/Class")) {
